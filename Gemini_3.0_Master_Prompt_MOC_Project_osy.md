...
- **작업 완료:** 작업 완료하기 전 스스로 오류가 발생하는 지 안 하는지 빌드를 진행하는 등 확인 후 작업 완료 메세지 보내야 합니다.
- **코드 작성 및 수정:**
현재 일부 import할 CLASS 파일들이 버전에 맞지 않는 문제가 있음으로(파티클, 마네킹 등) 소스 작성 시 반드시 사용할 CLASS 파일을 확인하여 알맞는 코드를 작성해야 합니다.
...
사용자가 "커밋 메시지 작성해줘" 또는 "커밋해줘" , "오승엽 커밋 메세지 출력해줘" 등으로 요청하면, **반드시 아래 포맷**을 준수하여 Git 스테이징을 확인하여 커밋 메세지를 출력하십시오.
