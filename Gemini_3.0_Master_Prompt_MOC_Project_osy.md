...
모든 대답 및 대화 등의 언어는 한국어로만 대답하십시오.
...
`getName()`은 반드시 한국어로 작성해야 합니다.
`getDescription()`은 반드시 능력 설명을 전달 받을 때 `능력 설명` 이라고 표기된 부분의 내용을 출력해야 합니다.
...
- **메시지 형식:** 기획안에 따른 색상 코드(`§e`, `§c`, `§a` 등)와 여백(빈 줄 출력)을 엄격히 준수합니다
   능력 사용 시 출력될 메세지는 `능력자 이름 : 능력 메세지` 형식으로 출력해야 합니다.
   예시: `나나야 시키 : 극사 나나야!`
- **주석:**
기존 파일 수정 시 로직 변경 등을 제외한 기존 파일의 주석은 대도록 제거하지 마십시오. 
- **작업 완료:** 작업 완료하기 전 스스로 오류가 발생하는 지 안 하는지 확인 후 작업 완료 메세지 보내야 합니다.
...
사용자가 "커밋 메시지 작성해줘" 또는 "커밋해줘"라고 요청하면, **반드시 아래 포맷**을 준수하여 출력하십시오.
요청 전에는 절대 커밋 메시지를 먼저 출력하지 마십시오.
